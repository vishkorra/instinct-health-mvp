...
import type { AccessCase, EpicWorkflowAction, EvidenceItem, PipelineItem, Role } from '../shared/types';
...
export function CaseWorkspace({ accessCase, activeRole, onRoleAction }: CaseWorkspaceProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <EpicWorkflowPanel accessCase={accessCase} activeRole={activeRole} onRoleAction={onRoleAction} />
...
function EpicWorkflowPanel({
  ...
  accessCase: AccessCase;
  activeRole: Role;
  onRoleAction: CaseWorkspaceProps['onRoleAction'];
}) {
  const roleActions = accessCase.epicWorkflowActions.filter((action) => action.owner === activeRole || action.owner === 'system');
  const actions = roleActions.length > 0 ? roleActions : accessCase.epicWorkflowActions.slice(0, 4);

  if (actions.length === 0) return null;

  return (
    <div className="panel overflow-hidden border-indigo-100 bg-white">
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-indigo-100 bg-gradient-to-r from-indigo-50 via-white to-cyan-50 px-5 py-4">
        <div>
          <div className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.18em] text-indigo-700">
            <Workflow className="h-4 w-4" />
            Selected Epic APIs in the workflow
          </div>
          <h3 className="mt-2 text-lg font-extrabold tracking-tight text-slate-950">
            Instinct converts the blocker into FHIR actions, not another PDF chase
          </h3>
          <p className="mt-1 max-w-4xl text-sm leading-6 text-slate-600">
            Live sandbox reads enrich the case. PA submission, attachment, inquiry, and notification packets are generated locally until an Epic/payer endpoint accepts those write operations.
          </p>
        </div>
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(260px,1fr))] gap-3 p-4">
        {actions.map((action) => (
          <WorkflowActionCard
            ...
            action={action}
            onRun={() => {
              const verb = action.status === 'simulated-ready' ? 'ran a sandbox simulation for' : 'generated';
              onRoleAction(`Instinct ${verb} ${action.api}.`);
            }}
          />
        ))}
      </div>
...
function WorkflowActionCard({ action, onRun }: { action: EpicWorkflowAction; onRun: () => void }) {
  const statusClass: Record<EpicWorkflowAction['status'], string> = {
    'live-read': 'bg-emerald-100 text-emerald-700',
    generated: 'bg-blue-100 text-blue-700',
    'simulated-ready': 'bg-amber-100 text-amber-700',
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-sm font-extrabold text-slate-950">{action.label}</div>
          <div className="mt-1 text-xs font-semibold text-slate-500">{action.api}</div>
        </div>
        <span className="rounded-full bg-slate-950 px-2 py-1 text-[11px] font-extrabold text-white">
          {action.method}
        </span>
      </div>
      <p className="mt-2 text-xs leading-5 text-slate-600">{action.detail}</p>
      <div className="mt-3 flex flex-wrap items-center gap-2">
        <span className={`rounded-full px-2 py-1 text-[11px] font-extrabold ${statusClass[action.status]}`}>
          {action.status}
        </span>
        <span className="rounded-full bg-white px-2 py-1 text-[11px] font-bold capitalize text-slate-600">
          {action.owner}
        </span>
      </div>
      <details className="mt-3 rounded-lg border border-slate-200 bg-white">
        <summary className="flex cursor-pointer items-center gap-2 px-3 py-2 text-xs font-extrabold text-slate-700">
          <Code2 className="h-3.5 w-3.5" />
          Payload preview
        </summary>
        <pre className="max-h-48 overflow-auto border-t border-slate-100 p-3 text-[11px] leading-5 text-slate-700">
          {action.payloadPreview}
        </pre>
      </details>
      <button type="button" className="secondary-button mt-3 w-full justify-center" onClick={onRun}>
        {action.status === 'simulated-ready' ? 'Run sandbox simulation' : 'Generate packet'}
      </button>
    </div>
  );
}
